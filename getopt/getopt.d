...
struct StdOutPrinter {
	static void write(string[][4] msg) {
		writeln(msg);
	}	
}

void getopt(O = StdOutPrinter,T...)(ref string[] args, T opts) {
    enforce(args.length,
    ...
    getoptImpl(args, cfg, helpOutput, opts);
...
private void getoptImpl(T...)(ref string[] args,
    ref configuration cfg, ref string[][4] helpOutput, T opts)
{
    static if (opts.length)
    {
        static if (is(typeof(opts[0]) : config))
        {
            ...
            return getoptImpl(args, cfg, helpOutput, opts[1 .. $]);
        }
        else
        {
            ...
            return getoptImpl(args, cfg, helpOutput, opts[2 .. $]);
        }
    }
    else
    {
